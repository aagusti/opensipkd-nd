...
# List #
########    
@view_config(route_name='notadinas-surat-outbox', renderer='templates/surat_keluar/list.pt',
             permission='notadinas-surat-outbox')
...
# Action #
##########    
@view_config(route_name='notadinas-surat-outbox-act', renderer='json',
             permission='notadinas-surat-outbox-act')
    ...
    if url_dict['act']=='grid':
        ...
        if u == 1:
            ...
            query = DBSession.query(Disposisi
                                     ...
                                     Disposisi.from_uid == req.user.id
                            )
            rowTable = DataTables(req, Disposisi, query, columns)
            return rowTable.output_result()
...
def save_request(values, request, row=None):
    ...
    if row:
        ...
        request.session.flash('Surat keluar id %s sudah diteruskan.' % row.id)   
    return row
	
def route_list(request):
    return HTTPFound(location=request.route_url('notadinas-surat-outbox'))
...
@view_config(route_name='notadinas-surat-outbox-add', renderer='templates/surat_keluar/add.pt',
             permission='notadinas-surat-outbox-add')
def view_add(request):
    ...
    if request.POST:
        if 'simpan' in request.POST:
            ...
            print '......................---------------...........',row			
            return HTTPFound(location=request.route_url('notadinas-surat-outbox-edit',id=row.id))
...
@view_config(route_name='notadinas-surat-outbox-edit', renderer='templates/surat_keluar/edit.pt',
             permission='notadinas-surat-outbox-edit')
...
@view_config(route_name='notadinas-surat-outbox-forward', renderer='templates/surat_keluar/forward.pt',
             permission='notadinas-surat-outbox-forward')
...
# Delete #
##########    
@view_config(route_name='notadinas-surat-outbox-delete', renderer='templates/surat_keluar/delete.pt',
             permission='notadinas-surat-outbox-delete')
def view_delete(request):
    ...
    if request.POST:
        if 'hapus' in request.POST:
            msg = 'Surat keluar ID %d sudah dihapus.' % (row.id)
            q.delete()
            DBSession.flush()
